...
- **Bot framework**: Telegraf v4.16.3 (long polling, no webhooks)
...
```
src/
  index.ts              # Entrypoint: bot setup, middleware chain, router
  ...
  services/
    ...
    scheduler.ts        # Daily 9:00 AM expiration alerts via node-cron
tests/
